...
export default function AdminPlayerMetricsPage() {
  const { user, isLoaded } = useUser();
  const params = useParams();
  const searchParams = useSearchParams();

  const targetUserId = String(params.userId || "");
  const targetUserName = searchParams.get("name") || "Player";
    ...
    async function load() {
      ...
      try {
        ...
        const gamesRes = await fetch(
          `${API_BASE}/games/my?user_id=${targetUserId}`,
          { cache: "no-store" }
        );
        ...
        for (const game of finalizedGames) {
          ...
          for (const hand of scoreboard.hand_summary) {
            const total = Number(hand.totals[targetUserId] ?? 0);
          ...
          for (const hand of scoreboard.hands) {
            const me = hand.players.find((p) => p.player_id === targetUserId);
            if (!me) continue;
          ...
          for (const roleRow of roleCards as any[]) {
            ...
            if (!participantIds.includes(targetUserId)) continue;
